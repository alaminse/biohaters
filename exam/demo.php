        ...
        <table class="hc_table hc_exam_table">
            ...
            <tbody>
                ...
                if (isset($_GET['filter_exam']) && $_GET['filter_exam'] != '') {
                    ...
                    if (isset($result['my_courses'])) {
                        // fetch exam
                        $select_exam = "SELECT * FROM hc_exam WHERE course_id IN ($total_course_id) AND status = 0 AND is_delete = 0 ORDER BY course_id DESC";
                        $sql_exam = mysqli_query($db, $select_exam);
                        $num_exam = mysqli_num_rows($sql_exam);
                    }
                }
                    ...
                    while ($row_exam = mysqli_fetch_assoc($sql_exam)) {
                        ...
                        if ($now >= $exam_date) {
                            ...
                            <tr>
                                ...
                                <td><?php if (($exam_cq == 1) && ($now >= $exam_date) && ($now <= $exam_valid_time)) {
                                    ...
                                    if ($num_cq_attempt == 0) {
                                        ?>
                                        <br>
                                        <a href="<?= $base_url ?>cq-attempt/test.php?exam=<?= $exam_id ?>" class="m_auto ep_flex ep_start button btn_sm text_sm">Start Exam</a>
                                        <?php 
                                    } else {
                                        $row_cq_attempt = mysqli_fetch_assoc($sql_cq_attempt);
                                        
                                        $cq_attempt_submitted_pdf       = $row_cq_attempt['submitted_pdf'];
                                        $cq_attempt_checked_pdf         = $row_cq_attempt['checked_pdf'];
                                        ...
                                        if ($cq_attempt_submitted_pdf == '' || $cq_attempt_submission_status == '' || $cq_attempt_submit_time == '') {
                                            ?>
                                            <br>
                                            <a href="<?= $base_url ?>cq-attempt/test.php?exam=<?= $exam_id ?>" class="m_auto ep_flex ep_start button btn_sm text_sm">Continue Exam</a>
                                            <?php 
                                        }
                                    }
                                } else {
                                    ...
                                    if ($num_cq_attempt > 0) {
                                        ...
                                        $cq_attempt_submitted_pdf       = $row_cq_attempt['submitted_pdf'];
                                        $cq_attempt_checked_pdf         = $row_cq_attempt['checked_pdf'];
                                        if ($cq_attempt_checked_pdf != '') {
                                            ?>
                                            <br>
                                            <a href="<?= $base_url ?>cq-attempt-details/?exam=<?= $exam_id ?>" class="m_auto ep_flex ep_start button btn_outline btn_sm text_sm">View Details</a>
                                            <?php 
                                        }
                                    } else { echo 'N/A'; }
                                }?></td>
                                ...
                                <td>
                                    <div class="btn_grp">
                                        ...
                                        if ($num_exam_attempt == 0) {
                                            ...
                                            ?>
                                            <a href="<?= $base_url ?>attempt-details/?exam=<?= $exam_id ?>" class="ep_flex ep_start button btn_outline btn_sm text_sm">View Details</a>
                                            <a href="<?= $base_url ?>exam-score/?exam=<?= $exam_id ?>" class="ep_flex ep_start button btn_outline btn_sm text_sm">Scoreboard</a>
                                            <?php 
                                        }?>
                                    </div>
