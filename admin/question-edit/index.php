...
<?php if (isset($_GET['question'])) {
    ...
    // get question data
    $select_question_data  = "SELECT * FROM hc_question_bank WHERE id = '$question' AND is_delete = 0";
    ...
    $select_options  = "SELECT * FROM hc_question_bank_option WHERE question = '$question'";
    ...
    if (isset($_POST['edit'])) {
        ...
        $newTopics = $_POST['topics'];
        $newExplanation = $_POST['explaination'];
        ...
        $updateQuery = "UPDATE hc_question_bank SET title = '$newTitle', chapter = '$newChapter', topic = '$newTopics', explaination = '$newExplanation', level = '$newLevel', academic = '$newAcademic', medical = '$newMedical' WHERE id = $questionId";
        ...
        if ($result) {
            ...
            foreach ($question_data['options'] as $option) {
                ...
                $updateOptionQuery = "UPDATE hc_question_bank_option SET option_name = '$newOptionText', is_correct = $isCorrect WHERE id = $optionId";
        
                mysqli_query($db, $updateOptionQuery);
            }?>
            <script type="text/javascript">
                window.location.href = '../question-bank/?chapter=<?= $newChapter ?>';
            </script>
            <?php 
        }
    }?>
<main>
    ...
    <div class="ep_section">
        <div class="ep_container">
            <h4 class="welcome_admin_title">Question Edit</h4>
        </div>
    </div>
        ...
        <div class="ep_container">
            ...
            <form action="" method="post" class="double_col_form" enctype="multipart/form-data">
                ...
                <div class="option_container grid_col_3">
                    <?php foreach ($question_data['options'] as $option) { ?>
                        ...
                        <div class="option_card">
                            ...
                            <input type="text" id="" name="options_<?= $option['option_id'] ?>" placeholder="" value="<?= $option['option_text'] ?>" <?php if ('True/False' == $question_data['type']) { echo 'readonly=""'; }?> required>
                        </div>
                    <?php } ?>
                </div>
                ...
                <div class="levels">
                    ...
                    <div>
                        <input type="checkbox" name="academic" value="academic" id="academic"
                            <?php if ($question_data['academic'] == '1') { echo 'checked'; } ?>>
